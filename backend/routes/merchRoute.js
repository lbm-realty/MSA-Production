...
router.post("/add-merch", authenticateToken, upload.single("imageFile"), async (req, res) => {
    try {
      ...
      if (saved) return res.send({ message: "Merch saved" });
      else return res.status(404).send({ message: "An error occurred" });
    } catch (err) {
      console.log(err);
...
router.put("/edit-merch/:id", authenticateToken, upload.single("imageFile"), async (req, res) => {
    try {
      ...
      if (updateData) res.send({ message: "Entry updated successfully!" });
